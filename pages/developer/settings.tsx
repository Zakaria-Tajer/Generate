...
const Settings = () => {
    ...
    useEffect(() => {
        ...
        req.open("POST", `${process.env.NEXT_PUBLIC_API_URL_Generate}api/getSelectedDev`, true);
        req.onload = () => {
            if (req.readyState === XMLHttpRequest.DONE) {
                if (req.status === 200) {
                    if (req.response.trim() == '') {
                        ...
                        const { data: { FirstName, LastName, img } } = data
                        setImg(img)
                        setFname(FirstName)
                        setLname(LastName)
                        console.log(data);
        ...
        req.onload = () => {
            if (req.readyState === XMLHttpRequest.DONE) {
                if (req.status === 200) {
                    ...
                    if (bodyMessage == "No user found") {
                        setIsData(true)
                    } else if (bodyMessage == "success") {
                        const { data: { Email_Address, Phone_number, Username } } = data
                    ...
                    <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                        <div className="md:flex space-y-5 md:space-y-0 ml-4 2xl:ml-[14.5rem] md:space-x-6 md:ml-5 lg:w-3/4 lg:mx-auto ">
                            ...
                            <div className="lg:space-x-7">
                                <label htmlFor="" className="xl:ml-7 block mb-1 2xl:ml-8 md:ml-0 font-poppins">
                                    Last name
                                </label>
                                <div className="w-96 md:w-80 py-2 px-6 font-poppins border-2 outline-none rounded-md">
                                    <h1>{lname}</h1>
                        ...
                        <div className="mt-6 pl-6 lg:pl-32 xl:pl-44 xl:pt-4 2xl:pl-[14.5rem] 2xl:pt-2 flex items-center space-x-10">
                            <div>
                                ...
                                <div className="w-14 h-14 rounded-full bg-blue-400">
                                    <img
                                        src={`http://localhost:8000/adminUploads/${img}`}
...
Settings.PageLayout = DevLayouts
