...
export const ClientNotifications = () => {
  ...
  const Notifications = useSelector(
    (state: NotificationModerator) => state.CLientImage
  );
  ...
  return (
    <>
      {arr.map((item, i) => (
        <div
          key={i}
          className="w-full max-w-lg relative p-4 mx-auto text-gray-900 bg-white rounded-lg shadow dark:bg-gray-800 dark:text-gray-300"
          role="alert"
        >
          ...
          {isShowing && (
            <motion.div
              className="left-0 absolute top-[8.5rem] w-full max-w-lg p-4 text-gray-900 bg-white rounded-lg shadow dark:bg-gray-800 dark:text-gray-300"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              <div className="py-3 border-b-[1px] flex space-x-3">
                <div className="w-14 h-14 rounded-full">
                  <img
                    ...
                    height="14"
                    alt="profile pic"
                    // src={`http://localhost:8000/uploads/${item.CLientImage}`}
                  />
                </div>
                <div className="">
                  {/* <h1 className="text-lg font-poppins">{`${item.E_mail}`}</h1> */}
